...
class latticePlanner:
    ...
    def latticePlanner(self,ref_path, vehicle_status):
        ...
        if len(ref_path.poses) > look_distance :
            ...
            lane_off_set = [-2.4, -1.6, -0.8, 0.8, 1.6, 2.4]
            ...
            # Path 생성 방식은 3차 방정식을 이용하며 lane_change_3 예제와 동일한 방식의 경로 생성을 하면 됩니다.
            # 생성된 Lattice 경로는 out_path 변수에 List 형식으로 넣습니다.
            # 충돌 회피 경로는 기존 경로를 제외하고 좌 우로 3개씩 총 6개의 경로를 가지도록 합니다.
                
            for end_point in local_lattice_points :
            '''
            x1 = local_ego_vehicle_position[0][0]
            y1 = local_ego_vehicle_position[1][0]
            x_interval = 0.3
            for end_point in local_lattice_points :
                x2 = end_point[0]
                y2 = end_point[1]
                lattice = Path()
                lattice.header.frame_id='/map'
                tmp_mat=np.array([[x1**3,x1**2,x1,1],[x2**3,x2**2,x2,1],[3*x1**2,2*x1,1,0],[3*x2**2,2*x2,1,0]])
                tmp_det_mat=np.linalg.inv(tmp_mat)
                coef=tmp_det_mat.dot(np.array([[y1],[y2],[0],[0]]))
                for i in np.arange(x1,x2,x_interval) :
                    global_tmp = trans_matrix.dot(np.array([[i],[coef[0][0]*i**3+coef[1][0]*i**2+coef[2][0]*i+coef[3][0]],[1]]))
                    tmp_pose = PoseStamped()
                    tmp_pose.pose.position.x = global_tmp[0][0]
                    tmp_pose.pose.position.y = global_tmp[1][0]
                    tmp_pose.pose.position.z = 0
                    tmp_pose.pose.orientation.x = 0
                    tmp_pose.pose.orientation.y = 0
                    tmp_pose.pose.orientation.z = 0
                    tmp_pose.pose.orientation.w = 0
                    lattice.poses.append(tmp_pose)
                out_path.append(lattice)
