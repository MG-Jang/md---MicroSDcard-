    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            if self.is_odom == True and self.cluster_status == True:
                ...
                '''
                trans_matrix = np.array([[cos(self.vehicle_yaw),-sin(self.vehicle_yaw),self.vehicle_pos_x],[sin(self.vehicle_yaw),cos(self.vehicle_yaw),self.vehicle_pos_y],[0,0,1]])
                ...
                for num,i in enumerate(self.cluster_data.poses) :
                    ...
                    '''
                    local_result = i
                    global_result = trans_matrix.dot(np.array([[i.position.x],[i.position.y],[1]]))
                    # print(global_result)
                    tmp_x = global_result[0][0]
                    tmp_y = global_result[1][0]

                    # print(self.vehicle_pos_x,self.vehicle_pos_y)
                    # print(tmp_x,tmp_y)

                    # cluster_obj_data_npc = ObjectStatus()
                    # cluster_obj_data_npc.type = 1
                    # cluster_obj_data_npc.position.x = tmp_x
                    # cluster_obj_data_npc.position.y = tmp_y
                    # cluster_obj_data_npc.position.z = 1.
                    # cluster_obj_data.npc_list.append(cluster_obj_data_npc)

                    cluster_obj_data_obstacle = ObjectStatus()
                    cluster_obj_data_obstacle.type = 2
                    cluster_obj_data_obstacle.position.x = tmp_x
                    cluster_obj_data_obstacle.position.y = tmp_y
                    cluster_obj_data_obstacle.position.z = 1.0
                    cluster_obj_data.obstacle_list.append(cluster_obj_data_obstacle)
