...
def input_control():
    ...
    # ctrl_mod = input("control mode [1340] : ")
    # if(not ctrl_mod) : event_info.option = 0
    # else : 
    #     event_info.option = 1
    #     event_info.ctrl_mode = ctrl_mod
    # gear_mod = input("Enter gear mod [1234-10] : ")
    # if(gear_mod!=0):
    #     event_info.option += 2
    #     event_info.gear = gear_mod
    # lamp_mod = input("Enter Lamp mod [10] : ")
    # if(lamp_mod):
    #     event_info.option += 4
    #     lamp = Lamps()
    #     lamp.turnSignal = input("Enter turn signal [0123] : ")
    #     lamp.emergencySignal = input("Enter emergency signal [01] : ")
    #     event_info.lamps = lamp
    # chsim = input("change sim status [01] : ")
    # if(chsim) :
    #     event_info.option +=8
    #     event_info.set_pause = bool(input("pause[1] continue[0] : "))

    event_info.option = 1            # 옵션의 위치를 정해줌
    event_info.ctrl_mode = 3  #ctrl_mode = 1: Keyboard / 3: automode / 4 : cruisemode
    event_info.option += 2
    event_info.gear = 4 # gear = -1 : 이전 상태 유지 1 : P / 2 : R / 3 : N / 4 : D
    event_info.option += 4
    lamp = Lamps()
    lamp.turnSignal = 0  #  0:No Signal / 1:Left Signal / 2:Right Signal / 3:이전 상태 유지
    lamp.emergencySignal = 1  #0:No Signal / 1:Emergency Signal
    event_info.lamps = lamp
    event_info.option +=8
    event_info.set_pause = 0  #시뮬레이터 Pause 상태로 유지 / False : 시뮬레이터 Play상태로 전환
...
def srv_client():
    ...
    while not rospy.is_shutdown():
        try:
            ...
            seteventcontrol = input_control()
            result=ros_srv(seteventcontrol)
