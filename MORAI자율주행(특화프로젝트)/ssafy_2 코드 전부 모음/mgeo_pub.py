...
class get_mgeo :
    def __init__(self):
        ...
        # print(self.node_msg.points)
        # print(self.link_msg.points)
        rate = rospy.Rate(1) 
    ...
    def getAllLinks(self):
        ...
        for link_idx in self.links :
            # print(link_idx)
            # for node_idx in link_idx.split('-') :
            #     print(node_idx)
            #     for pnts in self.links[node_idx].points.tolist():
            #         print(pnts)
            #         tmp_pnts=Point32()
            #         tmp_pnts.x,tmp_pnts.y,tmp_pnts.z=pnts
            #         all_link.points.append(tmp_pnts)        
            for pnts in self.links[link_idx].points.tolist() :
                # print(pnts)
                tmp_pnts=Point32()
                tmp_pnts.x,tmp_pnts.y,tmp_pnts.z=pnts
                all_link.points.append(tmp_pnts)
    ...
    def getAllNode(self):
        ...
        for node_idx in self.nodes :
            tmp_pnts=Point32()
            tmp_pnts.x,tmp_pnts.y,tmp_pnts.z=self.nodes[node_idx].point.tolist()
            # print(tmp_pnts)
            all_node.points.append(tmp_pnts)
            # all_node.points.append(self.nodes[node_idx].point.tolist())
        return all_node
