...
class LL2UTMConverter:
    ...
    def navsat_callback(self, gps_msg):
        ...
        utm_msg.data=[self.x,self.y]
        os.system('clear')
        rospy.loginfo('lat : {}'.format(self.lat))
        rospy.loginfo('lon : {}'.format(self.lon))
        rospy.loginfo('umt X : %11.3f' % self.x)
        rospy.loginfo('umt Y : %11.3f' % self.y)
    ...
    def convertLL2UTM(self):
        ...
        xy_zone = self.proj_UTM(경도 데이터, 위도 데이터)
