...
class pure_pursuit :
    def __init__(self):
        ...
        self.lfd = 5      #초기값: 5
        self.min_lfd=5    #초기값: 5
        self.max_lfd=27
        self.lfd_gain = 0.7    #속도에따라 얼마나 멀리 본건지 
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_path == True and self.is_odom == True and self.is_status == True and self.is_object_info == True:
                ...
                tmp_velocity = self.adaptive_cruise_control.get_target_velocity(local_npc_info, local_ped_info, local_obs_info,
                                                                                                        self.status_msg.velocity.x, self.target_velocity/3.6)
                if tmp_velocity < self.target_velocity :
                    self.target_velocity = tmp_velocity
