...
def talker():
    ...
    while not rospy.is_shutdown():
        ...
        '''
        # my_name = student()
        # my_name.first_name = raw_input("Enter First Name : ")
        # my_name.last_name = raw_input("Enter Last Name : ")
        # my_name.age = int(raw_input("Enter Age : "))
        # my_name.score = int(raw_input("Enter Score : "))
        
        rospy.loginfo('\n my name : %s %s \n my age : %i \n SSAFY score : %i', my_name.first_name,my_name.last_name,my_name.age,my_name.score)
        #TODO: (5) /my_name 메세지 Publish 
        publisher.publish(my_name)
        count += 1
        rospy.loginfo("[%d] Send My Info",count)
        rate.sleep()
