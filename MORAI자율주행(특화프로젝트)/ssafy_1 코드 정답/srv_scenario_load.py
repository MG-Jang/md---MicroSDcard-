...
def loadscenario():
    if(bool(input("wanna load?[10] : "))):
        scenario_setting = ScenarioLoad()
        scenario_setting.file_name = "temp_scenario"
...
def srv_client():
    ...
    #publisher = rospy.Publisher("/ScenarioLoad",ScenarioLoad,queue_size=10)
   ...
   scenario_setting.file_name                      = 시나리오 파일의 이름 입력
    ...
    while not rospy.is_shutdown():
        try:
            ...
            result = ros_srv(scenario_setting)
            #scene = loadscenario()
            #if(scene!=False) : publisher.publish(scene)
