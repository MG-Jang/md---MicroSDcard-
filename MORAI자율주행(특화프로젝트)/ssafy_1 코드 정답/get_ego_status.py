...
def EgoStatus_callback(data):
    ...
    '''
    rospy.loginfo('position     : x = {0} , y = {1}, z = {2}'.format( Ego 차량의 x y z 위치 좌표를 입력합니다.))
    ...
    rospy.loginfo('accel        : {} deg'.format(data.accel))
    rospy.loginfo('brake        : {} deg'.format(data.brake))
    rospy.loginfo('wheel_angle  : {} deg'.format(data.wheel_angle))
...
def listener():
    ...
    rospy.Subscriber( 변수 1 , 변수 2 , EgoStatus_callback)
    '''
