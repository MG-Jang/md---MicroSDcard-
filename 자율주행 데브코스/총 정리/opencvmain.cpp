...
int canny_edge_high_threshold_ = 80;
float hough_line_slope_range_ = 10.0;
...
static const bool kRightLane = true; //false?
...
std::pair<int, int> process_image(const cv::Mat& image) {
    ...
    int tmp = 0;
    // Image Preprocessing
    cv::cvtColor(image, gray_image, cv::COLOR_BGR2GRAY);  // 흑백으로 전환 
    cv::cvtColor(image, HSV, cv::COLOR_BGR2HSV);          // HSV변환
    ...
    cv::imshow("canny", canny_image); // hough 출력
    cv::imshow("HSV", HSV);           // HSV 출력
    ...
    //std::cout << "check_rpos: " << rpos << std::endl;


    // 내가 수정한 부분
    if (rpos == 0) rpos = lpos + 520;
    if (lpos == 0) lpos = rpos - 520;
    ...
    if (flag == 0) {
        ...
        std::cout << "둘다0!!!" << "lpos: " << lpos << ", rpos: " << rpos << std::endl;
    }
...
int main()
{
    ...
    cv::VideoCapture cap("Sub_project.mp4");
    ...
    while (true) {
        ...
        std::cout << "lpos77: " << lpos << ", rpos: " << rpos << ", mpos: " << "none" << std::endl;
        //draw_lines;
        int x = (lpos + rpos) / 2;
